...
struct Graph {
    ...
    Graph(int32_t N, int32_t K) : N(N), K(K), data((node_t *)align_alloc((size_t)N * K * sizeof(node_t), true, -1)) {}
    ...
    void prefetch(int32_t u, int32_t lines) const { mem_prefetch((char *)edges(u), lines); }

    void initialize_search(NeighborPoolConcept auto &pool, const ComputerConcept auto &computer) const {
        if (initializer) {
