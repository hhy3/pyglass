...
concept NeighborPoolConcept = requires(Pool pool, int32_t u, typename Pool::dist_type dist, int32_t *ids, float *dis,
...
struct Bitset {
    ...
    void set(int i) { data[i / block_size] |= (Block(1) << (i & (block_size - 1))); }
...
struct LinearPool {
    ...
    LinearPool(int n, int ef, int capacity) : nb(n), ef_(ef), capacity_(capacity), data_(capacity_ + 1), vis(n) {}
...
struct FlagNeighbor {
    ...
    FlagNeighbor(int id, float distance, bool f) : id(id), distance(distance), flag(f) {}

    inline friend bool operator<(const FlagNeighbor &lhs, const FlagNeighbor &rhs) {
        return lhs.distance < rhs.distance || (lhs.distance == rhs.distance && lhs.id < rhs.id);
    }
};
