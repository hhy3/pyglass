...
@dataclass
class IndexConfig:
    """Configuration for index parameters."""
    index_type: str
    R: int
    L: int = 0
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], build_quant: str) -> IndexConfig:
        """Create IndexConfig from dictionary."""
        return cls(
            index_type=data["index_type"],
            R=data["index_args"]["R"],
            L=data["index_args"].get("L", 0),
...
class Glass:
    ...
    def _build_ivf_index(self, X: np.ndarray) -> None:
        ...
        kmeans = KMeans(
            n_clusters=self.index_config.R, 
            random_state=0
        ).fit(X[:10000])
        print("KMeans done")
...
def calculate_recall(results: np.ndarray, ground_truth: np.ndarray) -> float:
    ...
    correct = sum(
        np.intersect1d(results[i], ground_truth[i]).size 
        for i in range(nq)
    )
...
def print_benchmark_header(dataset_name: str, index_type: str, search_quant: str, topk: int) -> None:
    """Print benchmark header."""
    print(f"\ndataset: {dataset_name}, index: {index_type}, quantizer: {search_quant}, top{topk}")
    print(f"{'ef':>4} | {'concurrency':>12} | {'Recall (%)':>12} | {'Max QPS':>12} | "
          f"{'P99 Latency (ms)':>20} | {'Avg Dist Comps':>18}")
    print("-" * 90)


def print_benchmark_results(ef: int, concurrency: int, results: Dict[str, float]) -> None:
    """Print benchmark results."""
    print(f"{ef:4d} | {concurrency:12d} | {results['recall']*100.0:12.2f} | "
...
def main() -> None:
    ...
    # Load configuration
    config_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("examples/config.yaml")
    ...
    for dataset_name in dataset_names:
        ...
        for topk in topks:
            ...
            for index_param in index_params:
                index_config = IndexConfig.from_dict(index_param, build_quant)
                ...
                for search_quant in search_quants:
                    ...
                    print(f"Memory usage of index: {glass_index.get_memory_usage():.2f}MB")
